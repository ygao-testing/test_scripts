...
resource "google_compute_instance" "agent_host" {
  name         = "${random_id.resource_prefix.hex}-rocky89"
  machine_type = "n2-standard-2"
  zone         = "us-central1-a"

  boot_disk {
    initialize_params {
      # Using Rocky Linux 8.9 image
      image = "rocky-linux-cloud/rocky-linux-8-v20250123"
    }
  }
  ...
  #!/bin/bash
  command -v cloud-init &>/dev/null || (sudo dnf update -y && sudo dnf install -y cloud-init && sudo reboot)
  CLOUD_INIT
