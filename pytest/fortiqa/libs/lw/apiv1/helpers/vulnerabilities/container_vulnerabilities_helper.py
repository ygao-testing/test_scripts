...
class ContainerVulnerabilitiesHelper:
    ...
    def list_current_pod_namespaces(self) -> list:
        ...
        logger.info("list_current_pod_namespaces()")
        ...
        logger.info(f"Pod namespaces: {json.dumps(query_card_response.json(), indent=2)}")
    ...
    def fetch_recent_eval_summary(self) -> list:
        ...
        assert query_card_response.status_code == 200, f"Failed to execute the card, error: {query_card_response.text}"
        logger.info(f"Recent eval summary: {json.dumps(query_card_response.json(), indent=2)}")
        return query_card_response.json()['data']
    ...
    def fetch_recent_eval_summary_with_active_container(self) -> list:
        ...
        assert query_card_response.status_code == 200, f"Failed to execute the card, error: {query_card_response.text}"
        logger.info(f"Recent eval summary with active container: {json.dumps(query_card_response.json(), indent=2)}")
        return query_card_response.json()['data']
    ...
    def wait_until_image_tag_appear(self, image_registry: str, ecr_repo: str, image_tag: str, timeout: int = 900) -> bool:
        ...
        if not image_found:
            ...
            return False
        logger.info(f"It took {time_passed} seconds until the image {image_tag} appears inside Container Vulnerability")
        return True
    ...
    def list_vulnerability_stats_cve_summary(self) -> list:
        ...
        logger.info("list_vulnerability_stats_cve_summary()")
        payload = deepcopy(self.payload_template)
        query_card_response = QueryCard(self.user_api).exec_query_card(card_name="Vuln_StatsSummaryCVETrend_MV", payload=payload)
        assert query_card_response.status_code == 200, f"Failed to execute the card, error: {query_card_response.text}"
        logger.info(f"Vulnerability CVE trend stats for images: {json.dumps(query_card_response.json(), indent=2)}")
        return query_card_response.json()['data']
    ...
    def fetch_container_vulnerability_summary_by_eval_guid(self, eval_guid: str) -> list:
        ...
        logger.info("fetch_container_vulnerability_summary_by_eval_guid()")
        payload = deepcopy(self.payload_template)
        ...
        assert query_card_response.status_code == 200, f"Failed to execute the card, error: {query_card_response.text}"
        logger.info(f"Detail vuln info for the container: {json.dumps(query_card_response.json(), indent=2)}")
        return query_card_response.json()['data']

    def fetch_recent_eval_by_image_id(self, image_id: str) -> list:
        """Fetch recent eval by a specific image id"""
        logger.info("fetch_recent_eval_by_image_id()")
        payload = deepcopy(self.payload_template)
        ...
        assert query_card_response.status_code == 200, f"Failed to execute the card, error: {query_card_response.text}"
        logger.info(f"Fetch recent evals by {image_id=}: {json.dumps(query_card_response.json(), indent=2)}")
        return query_card_response.json()['data']
