...
class AlertRules(LaceworkResource):
    """
    Class to interact with Alert Rules in Lacework API V1.
    ...
    def __init__(self, user_api, resource_payload=None) -> None:
        ...
        self._api_url = f"{user_api.url}/notifications/rules"
        self._resource_type = "Alert Rules V1"
        self._id_field = "MC_GUID"
        if resource_payload:
            self._resource_id = resource_payload.get("MC_GUID", "")
